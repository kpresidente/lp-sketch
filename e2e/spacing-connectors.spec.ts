...
test.describe('spacing and connector workflows', () => {
  ...
  test('arc auto-spacing places air terminals on a selected arc target', async ({ page }) => {
    ...
    await components.locator('button[title="Arc Auto-Spacing"]').click()
    await clickStage(page, { x: 300, y: 180 })
  ...
  test('auto-connectors support mechanical and cadweld modes', async ({ page }) => {
    ...
    const mechanical = page.locator('svg.overlay-layer g[data-symbol-type="cable_to_cable_connection"]')
    ...
    const cadweld = page.locator('svg.overlay-layer g[data-symbol-type="cadweld_connection"]')
  ...
  test('legend and general notes dialogs are reachable from placed items', async ({ page }) => {
    ...
    await tools.locator('button[title="Legend"]').click()
    ...
    await tools.locator('button[title="General Notes"]').click()
