...
- [x] Verify signed upload, Apple processing, and assignment to the existing iPad Development group. Report the actual build number and ask for physical Apple Pencil checks, with Insignia as an additional compatibility test.
...
- Apple processed build `13.1` successfully (`VALID`, resource `fb98df54-6687-4a78-88b0-e025e946e541`). After the user signed back in, the previously approved standard-encryption/no-France answers were saved in App Store Connect, and the build was assigned to the existing `iPad Development` group. The group shows one tester and three builds, with `0.1.0 (13.1)` **Testing**; the API independently confirms `IN_BETA_TESTING`. Physical checks remain pending for resting a palm before/during pen input, held palms after pen lift, fresh two-finger navigation, controls after navigation, and Insignia compatibility.
