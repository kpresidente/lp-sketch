...
- Build `0.1.0 (5.1)` is assigned to the `iPad Development` internal group with manual build assignment and the account owner as its sole tester. App Store Connect confirms 1 Tester, 1 Build, and tester status Invited. Device installation and native runtime checks remain pending.
