...
The [Pencil-selection TestFlight workflow](https://github.com/kpresidente/lp-sketch/actions/runs/35383711604) passed on September 18, 2026, uploading **0.1.0 (30.1)** from `ca422057d48c14d438b7c7222a1b70102525414c`. Both native archives, IPA export, Apple validation, and upload succeeded, and all implementation PR checks passed. Availability in the existing `iPad Development` group is still pending: Apple's browser session expired, and the owner was asked to sign back in so the previously approved encryption answers and group assignment can be completed. The first public API check immediately after upload did not yet return the build. Do not treat upload success as TestFlight availability; recheck processing and `IN_BETA_TESTING` after finishing these steps.
