...
The [palm-rejection workflow](https://github.com/kpresidente/lp-sketch/actions/runs/35335767694) passed on September 18, 2026, uploading **0.1.0 (13.1)** from `b8e092e5392b99615d38d325a1276504c88168c2`. Both archives, IPA export, Apple validation, and upload succeeded. Apple processed the build successfully; it is awaiting the encryption declaration and assignment to `iPad Development`. The browser session expired, and the existing Developer API key cannot change the encryption setting. The account owner has been asked to sign back in so the previously approved standard-encryption/no-France answers can be saved. Build `9.1` remains the verified available build until this last step is complete.
