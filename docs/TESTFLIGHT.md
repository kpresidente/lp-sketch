...
An unsigned build proves native compilation. It cannot be installed through TestFlight and does not prove iPad runtime behavior. Apple membership is active, the permanent bundle ID is `app.sslr.lpsketch`, and the LP Sketch app record, distribution certificate, and App Store provisioning profile are registered. A signed upload remains unverified until the upload key is installed and the manual workflow succeeds.
