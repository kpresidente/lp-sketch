...
On September 17, 2026, the account owner confirmed opening the TestFlight build on a physical iPad, importing a PDF, and drawing elements over it. This verifies the initial installation and those native runtime paths. PDF export, offline reopening, project recovery, rotation, and updating to a subsequent build remain to be checked. An unsigned build proves native compilation; it cannot be installed through TestFlight.
