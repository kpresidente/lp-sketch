...
The initial one-finger pan update added a mobile-only **Tools → One-finger pan** switch, initially off and persisted on the device. It preserves Pencil editing, two-finger pan/zoom, pending conductor endpoints, and browser touch editing. Both application builds, all 413 unit/integration tests, 26 browser E2E cases, and seven packaged-mobile Chromium cases passed. Independent review identified one cancelled-pinch continuation issue; red-first regression tests reproduced it, the fix passed all 106 interaction tests and the full suite, and focused review confirmed no remaining blocker. A rendered check at 1180 × 820 verified switch placement, persistence after reload, and a clean console. The owner subsequently tested the feature on the iPad and reported that it worked perfectly smoothly.

The [one-finger pan workflow](https://github.com/kpresidente/lp-sketch/actions/runs/35354553061) passed on September 18, 2026, uploading **0.1.0 (20.1)** from `492c7b93c9c18c7fa3ea0fdc7e9233ee16a51423`. Both archives, IPA export, Apple validation, and upload succeeded. Apple processed build resource `6ca769be-adf0-45dc-9fd2-22fa49ea4614` successfully; the previously approved standard-encryption/no-France answers cleared Missing Compliance. The build is assigned to the existing `iPad Development` group, and the App Store Connect API confirms `IN_BETA_TESTING`. All implementation PR checks passed. The PR remains a draft, unmerged into browser production. This build uses the optional **Tools → One-finger pan** switch.

Following the successful device test, the owner requested permanent one-finger navigation. The current source enables it automatically in the mobile app and removes the switch and stored-preference code. An old saved off preference no longer disables panning. Pencil priority, palm filtering, two-finger pan/zoom, the movement threshold, and browser touch editing remain unchanged. These changes are not included in TestFlight build `20.1`.

For the permanent-navigation follow-up, both application builds, all 411 unit/integration tests (including 104 editor interaction cases), 26 browser E2E cases, and seven packaged-mobile Chromium cases passed. Coverage includes absent and legacy saved preferences, removal of the switch, navigation after reload, and the existing pen/palm and gesture-transition regressions. Independent review found no actionable regressions.

Build **0.1.0 (25.1)** adds a shared collapsible sidebar and includes permanent one-finger navigation. Its six section icons open one panel at a time over the canvas. Selecting a tool or component dismisses the panel; settings keep it open. An outside tap only dismisses it, including when tapping over the canvas or quick-access toolbar. Escape preserves unfinished drawing work, and the device remembers the collapsed layout.

Sidebar validation passed both application builds, all 418 unit/integration tests (including 111 editor interaction cases), 34 browser E2E cases, and 15 packaged-mobile Chromium cases. The final implementation commit `2a4db51` passed all PR checks. Rendered checks covered 1180 x 820 and 820 x 1180 with no browser console errors. Independent review caught keyboard focus escaping behind the flyout and overlap with Quick Access editing state; failing browser regressions reproduced both, and the fixes passed with no remaining review findings. Physical sidebar behavior still requires the device checks below.

The [sidebar TestFlight workflow](https://github.com/kpresidente/lp-sketch/actions/runs/35368938498) passed on September 18, 2026, from `2a4db51b83f0ec2cd90e9cf934b30cd7bd233d29`. Both archives, IPA export, Apple validation, and upload succeeded. Apple processed build resource `f3167cd3-bbc2-494b-8e92-89db9eb78ec2`; the previously approved standard-encryption/no-France answers were saved in App Store Connect and cleared Missing Compliance. The build is assigned to the existing `iPad Development` group, and the App Store Connect API confirms `IN_BETA_TESTING`. The PR remains a draft, unmerged into browser production.

The field-contrast update strengthens shared button/input outlines and icons, increases small labels, gives disabled controls readable explicit colors, adds a selected-material checkmark, and improves switch tracks, focus rings, and panel edges. Both application builds, 418 unit/integration tests, 34 browser E2E cases, 15 packaged-mobile Chromium cases, the production dependency audit, and all 25 contrast pairs passed locally. Rendered checks at 1440 x 1000, 1366 x 1024, and 820 x 1180 verified flyout fit, label fit, selected/disabled states, and a clean console. The contrast audit now runs in CI. Physical sunlight readability remains unverified: compare available and disabled controls, on/off switch positions, and selected tools/materials indoors and outdoors at a consistent screen brightness.

The [field-contrast TestFlight workflow](https://github.com/kpresidente/lp-sketch/actions/runs/35376173621) passed on September 18, 2026, uploading **0.1.0 (27.1)** from `8bc113653cee7efd2a04c21fcdb444d840ab2400`. Both archives, IPA export, Apple validation, and upload succeeded. Apple processed build resource `2d0db86f-8a97-4fa2-9bcb-2a187a0babff`. The previously approved standard-encryption/no-France answers were saved in App Store Connect; the build was assigned to the existing `iPad Development` group, and the public API confirmed `IN_BETA_TESTING`. The public compliance PATCH returned HTTP 405, so compliance and group assignment were completed through the signed-in UI. All implementation PR checks passed, and independent contrast review found no actionable issues. The browser preview passed a rendered smoke check; the PR remains a draft, unmerged into browser production.

The Pencil-selection update adds an 8 CSS-pixel movement threshold in the mobile app, with immediate selection and no timed hold. Small pen slides leave object, group, and handle geometry unchanged and add no undo step; crossing the threshold starts the usual drag and permits fine positioning back inside the initial area. Activation uses client coordinates so zoom, snapping, and canvas layout shifts do not affect the threshold. Browser input and mobile mouse input remain immediate. Both builds, all 433 unit/integration tests (126 editor interaction cases), 35 browser E2E cases, 16 packaged-mobile Chromium cases, the production dependency audit, and all 25 contrast checks passed. Independent review found no actionable issues. A rendered 1366 x 1024 packaged-mobile check verified selection after jitter, deliberate drag, Undo, and a clean console. The owner subsequently confirmed that the Pencil-selection update worked perfectly on the iPad.

The [Pencil-selection TestFlight workflow](https://github.com/kpresidente/lp-sketch/actions/runs/35383711604) passed on September 18, 2026, uploading **0.1.0 (30.1)** from `ca422057d48c14d438b7c7222a1b70102525414c`. Both native archives, IPA export, Apple validation, and upload succeeded, and all implementation PR checks passed. After the owner restored the expired Apple session, the previously approved standard-encryption/no-France answers were saved and the build was assigned to the existing `iPad Development` group. The public API verified build resource `0ecd7c73-98b2-443c-872a-ddbd57a2f334` as `VALID`, with `usesNonExemptEncryption: false` and `IN_BETA_TESTING`, at 21:29 UTC on September 18. The public group-assignment POST returned HTTP 405, so assignment was completed through App Store Connect's signed-in UI. The PR remains a draft, unmerged into browser production.

The status-bar update reserves the visible iOS status-bar area above the native editor. Capacitor's Status Bar plugin uses the OS-reported height and reduces the web view's height so the bottom of the editor remains reachable. The bar has a light background with dark system icons. Shared editor/browser layout is unchanged. Device verification must cover portrait and landscape, opening a sidebar flyout near the top, access to bottom controls, and returning to the app after switching away.

Status-bar validation passed both application builds, mobile sync, all 433 unit/integration tests, 35 browser E2E cases, 16 packaged-mobile Chromium cases, six signing-configuration tests, and the production dependency audit. Sync registered Status Bar 8.0.3 in the generated native configuration and Swift package. Independent review found no actionable issues. These checks do not render the native status bar; the physical checks above remain necessary.

The [status-bar TestFlight workflow](https://github.com/kpresidente/lp-sketch/actions/runs/35398165197) passed on September 18, 2026, uploading **0.1.0 (36.1)** from `f461043d341852009a9b54e1d041bdbc08b3b7b7`. Both native archives, IPA export, Apple validation, and upload succeeded, and all implementation PR checks passed. Apple processed build resource `b11cbf1f-94ba-4330-928d-2b0a21e2f052`. The previously approved standard-encryption/no-France answers were saved through App Store Connect, and the build was assigned to the existing `iPad Development` group. At 21:52 UTC, the public API confirmed `VALID`, `usesNonExemptEncryption: false`, and `IN_BETA_TESTING`. Physical status-bar acceptance remains pending. The PR remains a draft, unmerged into browser production.

The sidebar-orientation update removes the shared narrow-window rule that moved the expanded sidebar above the workspace. Both browser and mobile now keep the full sidebar on the left in portrait and landscape, with the canvas using the remaining width. Collapse/expand and section flyouts retain their existing behavior. Both application builds, all 433 unit/integration tests, 35 browser E2E cases, and 16 packaged-mobile Chromium cases passed. Rendered checks covered 820 x 1180 portrait, 1180 x 820 landscape, and browser widths of 1024 and 1440, with no horizontal page overflow or console errors. Independent review found no actionable issues. Physical rotation still needs verification after installing the update.

The [sidebar-orientation TestFlight workflow](https://github.com/kpresidente/lp-sketch/actions/runs/35402919589) passed on September 18, 2026, uploading **0.1.0 (39.1)** from `577e9e836e42923a7bc2a1b39d7e27f95b6f0900`. Both native archives, IPA export, Apple validation, and upload succeeded, and all implementation PR checks passed. Apple processed build resource `65e2769b-51a8-4a2f-84fc-30fcb31ce0e0`. The previously approved standard-encryption/no-France answers were saved through App Store Connect, and the build was assigned to the existing `iPad Development` group. At 22:54 UTC, the public API confirmed `VALID`, `usesNonExemptEncryption: false`, and `IN_BETA_TESTING`. The deployed browser preview also passed a rendered portrait layout check. The PR remains a draft, unmerged into browser production.
...
- Lift your hand, then use two fingers to pan and pinch to zoom. Try this between line endpoints and with the Pan tool selected. A fresh single-finger drag should pan without editing the canvas.
...
The mobile browser preview exercises the same input policy, but Chromium tests cannot establish hardware palm rejection. Hand contacts moving before a pen contact may still look like an intentional navigation gesture; record any resulting movement during the device check. Line construction remains the existing endpoint-tap workflow in this build.
...
For one-finger pan testing:

- One-finger panning is always enabled in the mobile app, with no Tools switch or saved preference.
...
- Add a second finger to pinch or pan, then lift one finger and continue panning. Check for jumps at both transitions.
...
- Background and reopen the app, or interrupt an active gesture, then check that held contacts do not resume movement and fresh gestures work. Check that navigation works after an app restart.
- Try resting and repositioning your hand **before** the Pencil touches down. That contact can still look like an intentional finger drag; report any accidental movement and the sequence that caused it.

The checkpoint before one-finger panning is `cb21eab02f975d46598261d9b06ffbb33098b091` (TestFlight `17.1`). The owner verified one-finger panning in TestFlight `20.1`, recorded at checkpoint `df11c87`. The change making it permanent is kept in a separate commit.

For collapsible-sidebar testing:

- Collapse the sidebar using its heading button. Open each of the six section icons, switch between them, and expand back to the full sidebar.
- Select a tool or component with a finger or Pencil: the panel should close. Change switches, material, scale values, and layers: it should stay open.
- Open a panel while placing a conductor, then tap outside with the Pencil. The tap should close the panel without placing the next endpoint; the following tap should finish the conductor normally.
- Open a panel and tap over Undo or Delete. The first tap should only close the panel; a subsequent tap should operate the control.
- Rotate between landscape and portrait with the sidebar expanded, then repeat with it collapsed. Both layouts should stay on the left beside the canvas. Every flyout should fit on screen and scroll, and success/error messages should remain visible. Repeat by narrowing the browser window.
- Reopen the app and check that the collapse/expand choice persists. Repeat finger pan, two-finger zoom, and resting-hand checks after closing a panel.
