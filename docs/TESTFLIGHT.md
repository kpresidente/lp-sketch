...
Build **0.1.0 (17.1)** adds a fixed Delete trash button below Undo/Redo in the shared quick-access bar. It stays visible and is disabled unless there is an active selection in Select or Multi-Select mode and no editing dialog or quick-access editor is open. Deleting one or several selected objects uses the existing undo history. Browser/mobile builds, all 398 unit/integration tests, 26 browser E2E cases, and six packaged-mobile Chromium cases passed. Independent review caught a stale selected-mark reference after Clear All Marks; the focused fix passed all 91 interaction tests, both builds, and the mobile E2E suite, and the reviewer confirmed no remaining findings. A rendered check at 1180 × 820 verified button placement, selection, deletion, Undo, and a clean console. Physical iPad verification of this control is pending.
