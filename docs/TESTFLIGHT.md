...
Apple processing and the initial export-compliance declaration are complete. Build `0.1.0 (5.1)` is assigned to the `iPad Development` internal group, with the account owner as its sole tester and manual build assignment. App Store Connect confirms the tester is **Invited**. Device installation and native runtime checks remain to be performed on the iPad. An unsigned build proves native compilation; it cannot be installed through TestFlight.
